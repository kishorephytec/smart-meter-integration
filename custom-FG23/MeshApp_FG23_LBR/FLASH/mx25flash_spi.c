...
USART_TypeDef *temp_data;
void MX25_init( void )
{
  CMU_ClockEnable( cmuClock_GPIO, true );
    temp_data = MX25_USART;
#if defined(MX25_USART)
...
#endif
#if 0    //SPI PIN MODIFICATIONS
   /* IO config */
  GPIO_PinModeSet(gpioPortD, 14, gpioModePushPull, 0); // US1_CLK is push pull
	    GPIO_PinModeSet(gpioPortB, 14, gpioModePushPull, 1); // US1_CS is push pull
	    GPIO_PinModeSet(gpioPortD, 13, gpioModePushPull, 1); // US1_TX (MOSI) is push pull
	    GPIO_PinModeSet(gpioPortB, 13, gpioModeInput, 1);    // US1_RX (MISO) is input
#else
             GPIO_PinModeSet(gpioPortA, 0, gpioModePushPull, 0); // US1_CLK is push pull
	    GPIO_PinModeSet(gpioPortD, 13, gpioModePushPull, 1); // US1_CS is push pull
	    GPIO_PinModeSet(gpioPortB, 12, gpioModePushPull, 1); // US1_TX (MOSI) is push pull
	    GPIO_PinModeSet(gpioPortB, 13, gpioModeInput, 1);    // US1_RX (MISO) is input
...
#if 0  //SPI PIN  LOC MODIFICATIONS   
    MX25_USART->ROUTELOC0 =  (USART_ROUTELOC0_CLKLOC_LOC20) | // US1_CLK       on location 11 = PC8 
	                        (USART_ROUTELOC0_CSLOC_LOC6)  | // US1_CS        Manual
	                        (USART_ROUTELOC0_TXLOC_LOC21)  | // US1_TX (MOSI) on location 11 = PC6 
	                        (USART_ROUTELOC0_RXLOC_LOC7);   // US1_RX (MISO) on location 11 = PC7 
#else
     MX25_USART->ROUTELOC0 =  (USART_ROUTELOC0_CLKLOC_LOC30) | // US1_CLK       on location 30 = PA0
	                        (USART_ROUTELOC0_CSLOC_LOC18)  | // US1_CS        Manual  18 = PD13
	                        (USART_ROUTELOC0_TXLOC_LOC7)  | // US1_TX (MOSI) on location 11 = PC6 
	                        (USART_ROUTELOC0_RXLOC_LOC7);   // US1_RX (MISO) on location 11 = PC7 

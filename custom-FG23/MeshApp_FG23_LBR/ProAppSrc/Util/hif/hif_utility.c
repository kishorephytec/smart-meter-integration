...
////[kimbal]    
    ...
    uint8_t* p_data = (uint8_t*) app_bm_alloc( msg_len);
////[kimbal]
...
////[kimbal]
    ...
    {      
      ...
      memcpy( &(pBuffer->data[0]), ((uint8_t*)&msg_len), 2); 
			...
			if ( grp_id ==  p_curr_ser->group_id )
			{
			   ...
			   //call the hif2mac or hif2phy function based on grp_id
                                ...
                                if ( HIF_SEND_CONF == status )
				{
					...
					p_hif_data->app_call_back( RX_PACKET_EVENT );
					return true;
				}
				else
				{
...
//////[kimbal]
	if( NULL != p_hif_buff )
	{
//            if((p_hif_buff->data[2] == '$')&&(p_hif_buff->data[3] == '$'))
            {
                          ...
                          )
{
  ...
  if( TX_COMPLETE_EVENT == Event )
  {
    ...
    p_hif_data->app_call_back( TX_PACKET_SENT_EVENT);
  }

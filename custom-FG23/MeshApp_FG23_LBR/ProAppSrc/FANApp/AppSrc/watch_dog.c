...
void init_wdog(void) {
  ...
  /* Warning time is configured as 50% so as to provide enough time to the
   * user to observe it's output on the LCD.
   */
  wdog_config.warnSel = wdogWarnTime50pct;
  wdog_config.em4Block = false;
  wdog_config.swoscBlock = false;
  wdog_config.winSel = wdogIllegalWindowTime12_5pct; 
  /* wdog_config.resetDisable can be set to true
   * to prevent the system from being reset. In this case the TOUT interrupt from
   * the WDOG will occur and "System reset string will be displayed on the LCD
   */

  WDOGn_Init(WDOG0, &wdog_config);
  
  WDOGn_IntEnable(WDOG0, (WDOG_IF_TOUT));
...
volatile int int_flags = WDOGn_IntGet(WDOG0);

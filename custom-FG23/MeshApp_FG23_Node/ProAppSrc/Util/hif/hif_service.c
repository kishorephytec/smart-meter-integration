...
static void hif_service_notify( uint8_t evt , uint8_t ch );
...
extern sw_tmr_t uart_rx_data_poll_tmr;
...
void UART_poll_timer_expire_callback_to_App(void *uart_data);
#endif
...
void hif_service_init(hif_t* p_hif_data) //To be called from main
{
	...
	( 
        ...
        tmr_create_one_shot_timer
        (
            &(uart_rx_data_poll_tmr),
            (30 * 1000 ),  // Approx 30 Milli Sec 
            (sw_tmr_cb_t)&UART_poll_timer_expire_callback_to_App,
            NULL
        );
#endif     
...
#if(APP_USE_HIF_FOR_RAW_COMMUNICARION == 1)
#define MAX_UART_DATA_BUFF_SIZE 1500
uint8_t myData [ MAX_UART_DATA_BUFF_SIZE ] = {0};
uint16_t myDataIDx = 0;
static void hif_service_notify( uint8_t ev , uint8_t ch )
{
  
  if (ev == RX_PACKET_EVENT) 
  {
    myData[myDataIDx++] = ch;
...
extern int get_joing_state;
uint8_t state_join = 0;
#if(APP_USE_HIF_FOR_RAW_COMMUNICARION == 1)
void UART_poll_timer_expire_callback_to_App(void *uart_data)
{
   ...
   UART_Data_send_udp(&myData[0], myDataIDx);

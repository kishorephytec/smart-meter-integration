...
int UART_Data_Reading (void)
	{
	    ...
	    while( 1 )
	    {
	        ...
	        {
	            //printf("\n Got the data  - %x \n ", rx_char);
	        	process_uart_byte(rx_char,timestamp);
	        }//END of if condition...
	    }//END of while loop....
	    return EXIT_SUCCESS;
...
#if DEBUG
		int j=0;
	        printf("%s [INFO][RECEIVED DATA        ]: Received data to UART :",timestamp);
		for(j = 0;j < length;j++)
		   printf("%x ",write_bufer[j]);
		printf("\n");
#endif

	if(uart_file_discripter !=-1)
	{
		int write_count = write(uart_file_discripter,write_bufer,length);

		if(write_count != length)
		{
# if DEBUG
			printf("%s [INFO][ERROR                ]: Uart write count [%d] with error no [%d]\n\r",timestamp,write_count,errno);
			return EXIT_FAILURE;
#endif
		}
	}
	...
	{
# if DEBUG
			printf("%s [INFO][ERROR                ]: Uart failed to open file descriptor with error no [%d]\n\r",timestamp,errno);
			return EXIT_FAILURE;
#endif
	}
	return EXIT_SUCCESS;
